...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"], # Railway manejará los dominios de prod, dejamos abierto
...
# Cliente de ChromaDB en modo LECTURA de la carpeta que vas a subir
...
# 🐒 REEMPLAZO DE EMBEDDER: Ahora le pedimos el vector a Gemini directamente
def generar_embedding_con_gemini(texto: str) -> List[float]:
    if not client_ai:
        return [0.0] * 768 # Fallback por si acaso
    response = client_ai.models.embed_content(
        model="text-embedding-004", # El modelo oficial de Google para vectores
        contents=texto
    )
...
async def analyze_message(request: AnalyzeRequest):
    ...
    try:
        ...
        # Búsqueda semántica usando el vector de Google
        ...
        procesar_resultados(res, "Recomendados")
...
if __name__ == "__main__":
    import uvicorn
    # Railway inyecta la variable PORT automáticamente, así la leemos
